import os
import os.path                      # It is used for processing files from different places in the system
from os import path     
from Crypto.Cipher import AES       #processing files from different places in the system  & AES uses symmetric passwrd encryption, which involves the use of only one secret passwrd to cipher and decipher information.
from Crypto.Hash import SHA256      #SHA-256 is used for secure password hashing
from Crypto import Random           #Return a random byte string of length N
...
DEBUG = False
console = Console()
headerText = "mir233mhga1"


def encrypt(passwrd, source, encode=True):
    passwrd = SHA256.new(passwrd).digest()  # use SHA-256 over our passwrd to get a proper-sized AES passwrd
    IV = Random.new().read(AES.block_size)  # it generate IV
    encryptor = AES.new(passwrd, AES.MODE_CBC, IV)
    padding = AES.block_size - len(source) % AES.block_size  # calculate needed padding
    source += bytes([padding]) * padding  # Python 2.x: source += chr(padding) * padding
    data = IV + encryptor.encrypt(source)  # store the IV at the beginning and encrypt
    return base64.b64encode(data).decode() if encode else data

def decrypt(passwrd, source, decode=True):
    if decode:
        source = base64.b64decode(source.encode())
    passwrd = SHA256.new(passwrd).digest()  # use SHA-256 over our passwrd to get a proper-sized AES passwrd
    IV = source[:AES.block_size]  # extract the IV from the beginning
    decryptor = AES.new(passwrd, AES.MODE_CBC, IV)
...
def main():
	...
	if op==1:
		...
		cipher=""
		if password!="":
			cipher = encrypt(passwrd=password.encode(),source=message.encode())
		...
		if password!="":
			...
			print("cipher : ",cipher)
			try:
				decrypted = decrypt(passwrd=password.encode(),source=cipher)
			except Exception as e:
...
if __name__ == "__main__":
	...
	cprint(figlet_format('mini -  project', font='starwars'),'yellow', attrs=['bold'])
	print_credits()
	print()
	print("[bold]Welcome[/bold] to our Mini-Project. Here you can hide text inside an image. \nWe protect your texts with a password using AES-256.")
	print()
